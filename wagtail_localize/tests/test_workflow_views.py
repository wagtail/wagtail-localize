...
def create_test_pages(test):
    test.en_homepage = Page.objects.get(depth=2)
...
class TestCreateTranslationRequest(TestCase, WagtailTestUtils):
    ...
    def test_get_create_translation_request(self):
        response = self.client.get(
            reverse(
                "wagtail_localize:create_translation_request",
...
class TestTranslationListing(TestCase, WagtailTestUtils):
    ...
    def test_get(self):
        response = self.client.get(reverse("wagtail_localize_management:list"))
        ...
        self.assertContains(
            response,
            reverse(
                "wagtail_localize_management:detail",
