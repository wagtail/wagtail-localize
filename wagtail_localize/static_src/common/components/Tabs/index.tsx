    ...
    <div className='w-tabs'>
      <div className='w-tabs__wrapper'>
        <div className="w-tabs__list" role="tablist">
          {tabs.map(tab => {
            ...
            return (
              <a data-count={tab.numErrors || 0} onClick={onClick} aria-selected={tab.slug == currentTab} className="w-tabs__tab" href={`#tab-${tab.slug}`} id={`tab-label-${tab.slug}`} role="tab" aria-controls={`tab-${tab.slug}`}>
                {tab.label}
              </a>
            );
          })}
      ...
      <div className="tab-content">
        <CurrentTabContext.Provider value={currentTab}>
          <section aria-labelledby={`tab-label-${currentTab}`} id={`tab-${currentTab}`} role="tabpanel">
            {children}
          </section>
