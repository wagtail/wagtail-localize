...
def register_sites_menu_item():
    return RegionsMenuItem(_('Regions'), reverse('wagtail_localize_regions_admin:index'),
                         classnames='icon icon-site', order=602)


@hooks.register('register_permissions')
def register_permissions():
    return Permission.objects.filter(content_type__app_label='wagtail_localize_regions_admin',
