...
def import_file(request, translation_id=None):
    ...
    with transaction.atomic():
        ...
        translation.update_destination(user=request.user)
    ...
    return redirect(
        "wagtail_localize_management:detail", translation_id
    )
...
def translation_form(request, translation_id):
    ...
    with transaction.atomic():
        for segment in translation.source.stringsegment_set.all():
            string_translation = request.POST.get(f"segment-{segment.id}", "")
